...
import { IconButton, FormControl, FormControlLabel, FormLabel, Radio, RadioGroup } from '@mui/material';

const App = () => {
  const [foreground, setForeground] = useState("");
  const [background, setBackground] = useState("");
  const [grid, setGrid] = useState<{ fg: string; bg: string; label: string }[]>([]);
  ...
  function getColorType(color:string | null) {
  ...
  function convertRGBAtoRGB(rgba:string) {
  ...
  return (
    <div style={{ padding: "1rem", fontFamily: "Arial, sans-serif" }}>
      ...
      <FormControl>
      <FormLabel id="demo-radio-buttons-group-label">Color Format:</FormLabel>
        <RadioGroup
  ...
  >
     {["hex", "rgb", "hsl"].map((type) => (
      <FormControlLabel control={<Radio                 value={type} checked={colorType === type} 
      onChange={handleColorTypeChange} />} label={type.toUpperCase()} />
     ))}
      ...
      >
        {grid.map((color, index) => {
          ...
          return (
            ...
            >
              <div>Sample Text</div>
              <div style={{ fontSize: '0.8rem', marginTop: '0.5rem' }}>{color.label}</div>
